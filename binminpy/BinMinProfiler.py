...
class BinMinProfiler:
    ...
    def _call_and_log_likelihood(self, params):
        """Calls the likelihood, counts, and logs the sample. For serial use."""
        logL = self._call_likelihood(params)
        self._log_sample(params, logL)
        return logL
    ...
    def _find_initial_maxima(self, serial=False):
        ...
        if serial:
            ...
            tasks = [{'start_point': sp, 'bounds': self.bounds} for sp in start_points]
            results = self._dispatch_optimize_tasks(tasks)

            for res in results:
                if res['success']:
                    self.initial_maxima.append({'point': res['point'], 'logL': res['logL']})
                    if res['logL'] > self.global_max_logL:
                        self.global_max_logL = res['logL']
    ...
    def _initialize_from_warm_start_file(self, warm_start_file, serial=False):
        ...
        for grid_idx, candidate in best_candidates.items():
            if candidate['logL'] < roi_cutoff or grid_idx in self.population:
                continue
            ...
            for i, params in enumerate(scaled_samples):
                ...
                if i == closest_idx:
                    ...
                    eval_tasks.append({'params': full_params, 'pop_idx': i})
        ...
        points_to_evaluate = [task['params'] for atask in activation_tasks for task in atask['eval_tasks']]
        if serial:
            results = [self._call_and_log_likelihood(p) for p in points_to_evaluate]
        else:
            results = self._dispatch_eval_tasks(points_to_evaluate)

        result_idx = 0
        for atask in activation_tasks:
            for task in atask['eval_tasks']:
                atask['fitnesses'][task['pop_idx']] = results[result_idx]
                result_idx += 1

            grid_idx = atask['grid_idx']
            best_fitness = np.max(atask['fitnesses'])
            self.profile_likelihood_grid[grid_idx] = best_fitness

            self.population[grid_idx] = {
                'continuous_params': atask['continuous_params'],
                'fitnesses': atask['fitnesses'],
        ...
        if self.rank == 0:
            print(f"--- Activated {len(activation_tasks)} grid points from file. New Global Max logL: {self.global_max_logL:.4e} ---")

    def _activate_point(self, grid_idx, warm_start_params=None, serial=False):
        ...
        for params in scaled_samples:
            ...
            full_params[self.projection_dims] = self._get_grid_coords_from_indices(grid_idx)
            full_params[self.continuous_dims] = params
            points_to_evaluate.append(full_params)

        if serial:
            fitness_results = [self._call_and_log_likelihood(p) for p in points_to_evaluate]
        else:
            fitness_results = self._dispatch_eval_tasks(points_to_evaluate)

        for i, params in enumerate(scaled_samples):
            continuous_params[i] = params
            fitnesses[i] = fitness_results[i]
    ...
    def _initialize_population_from_maxima(self, serial=False):
        ...
        for maximum in self.initial_maxima:
            ...
            for neighbor_idx in self._get_valid_neighbors(grid_idx, include_center=True):
                self._activate_point(neighbor_idx, warm_start_params=point[self.continuous_dims], serial=serial)
        if self.rank == 0:
    ...
    def run(self, num_generations, max_num_to_evolve=None, plot_callback=None, plot_interval=10, skip_init_opt_on_warm_start=True):
        if self.size == 1:
            return self._serial_run(num_generations, max_num_to_evolve, plot_callback, plot_interval, skip_init_opt_on_warm_start)

        if self.rank == 0:
            return self._manager_run(num_generations, max_num_to_evolve, plot_callback, plot_interval, skip_init_opt_on_warm_start)
        else:
            self._worker_run()

    def _serial_run(self, num_generations, max_num_to_evolve=None, plot_callback=None, plot_interval=10, skip_init_opt_on_warm_start=True):
        return self._manager_run(num_generations, max_num_to_evolve, plot_callback, plot_interval, skip_init_opt_on_warm_start, serial=True)
    ...
    def _worker_run(self):
        ...
        while True:
            ...
            if task_type == "EVAL":
                result = self._call_likelihood(task["params"])
                response = {"result": result, "original_index": task["original_index"],
                            "params": task["params"], "calls": 1}
                self.comm.send(response, dest=0)
                ...
                def objective_func(params):
                    nonlocal calls
                    calls += 1
                    return -self._call_likelihood(params)
                ...
                response = {"result": result, "original_index": task["original_index"], "calls": calls}
                self.comm.send(response, dest=0)

    def _manager_run(self, num_generations, max_num_to_evolve=None, plot_callback=None, plot_interval=10, skip_init_opt_on_warm_start=True, serial=False):
        """Main entry point to run a sequence of projections."""
        fig, axes = None, None
        if plot_callback and self.rank == 0:
            try:
                import matplotlib.pyplot as plt
                plt.ion()
        ...
        for i, proj_config in enumerate(self.projections):
            ...
            if plot_callback and self.rank == 0:
                ...
                filename = f"plot__{self.likelihood_func_name}__{proj_dims_str}.png"
    ...
    def _shutdown_workers(self):
        """Sends a stop signal to all worker processes."""
        for i in range(1, self.size):
            self.comm.send("STOP", dest=i)

    def _dispatch_eval_tasks(self, points_to_evaluate):
        """Dispatches single point evaluation tasks to workers and collects results."""
        num_points = len(points_to_evaluate)
        if num_points == 0:
            return []

        self.likelihood_calls = 0 # Reset manager's count
        results = [None] * num_points
        tasks = [{'type': 'EVAL', 'params': p, 'original_index': i} for i, p in enumerate(points_to_evaluate)]

        task_index = 0
        results_count = 0

        for worker_rank in range(1, self.size):
            if task_index < num_points:
                self.comm.send(tasks[task_index], dest=worker_rank)
                task_index += 1

        while results_count < num_points:
            status = MPI.Status()
            response = self.comm.recv(source=MPI.ANY_SOURCE, tag=MPI.ANY_TAG, status=status)
            worker_rank = status.Get_source()

            results[response['original_index']] = response['result']
            self.likelihood_calls += response['calls']
            self._log_sample(response['params'], response['result'])
            results_count += 1
    ...
    def _dispatch_optimize_tasks(self, optimization_tasks):
        """Dispatches optimization tasks to workers and collects results."""
        ...
        for worker_rank in range(1, self.size):
            if task_index < num_tasks:
                task = optimization_tasks[task_index]
                task['type'] = 'OPTIMIZE'
                task['original_index'] = task_index
                self.comm.send(task, dest=worker_rank)
                task_index += 1
        ...
        while results_count < num_tasks:
            ...
            results[response['original_index']] = response['result']
            self.likelihood_calls += response['calls']
            # Note: samples from within optimizer are not logged individually for now.
            # Only the final result is available here.
            res = response['result']
            if res['success']:
                self._log_sample(res['point'], res['logL'])
    ...
    def _run_single_projection(self, num_generations, max_num_to_evolve, plot_callback, plot_interval,
                               ...
                               projection_config, fig=None, axes=None, serial=False):
        """Internal method to run the sampling loop for one projection."""
        if not skip_initial_optimizations_on_warm_start:
            self._find_initial_maxima(serial=serial)

        self._initialize_from_warm_start_file(warm_start_file, serial=serial)

        self._initialize_population_from_maxima(serial=serial)
            ...
            for grid_idx in indices_to_process:
                ...
                for i in range(self.pop_per_grid_point):
                    ...
                    if use_neighbor_mutation:
                        ...
                        if not pbest_archive: pbest_archive = parent_pool
                        ...
                        if len(potential_diff) < 2: continue
                    ...
                    de_tasks.append({'grid_idx': grid_idx, 'pop_idx': i, 'params': full_params,
                                     'trial_params': trial_params, 'F': F_i, 'CR': CR_i})

            if serial:
                fitness_results = [self._call_and_log_likelihood(task['params']) for task in de_tasks]
            else:
                fitness_results = self._dispatch_eval_tasks([task['params'] for task in de_tasks])

            for i, task in enumerate(de_tasks):
                trial_fitness = fitness_results[i]
                grid_idx = task['grid_idx']
                pop_idx = task['pop_idx']
                grid_state = self.population[grid_idx]

                if trial_fitness > grid_state['fitnesses'][pop_idx]:
                    grid_state['continuous_params'][pop_idx] = task['trial_params']
                    grid_state['fitnesses'][pop_idx] = trial_fitness
                    successful_F.append(task['F'])
                    successful_CR.append(task['CR'])
                ...
                if state['best_fitness'] > (self.global_max_logL - self.roi_threshold):
                    for neighbor_idx in self._get_valid_neighbors(grid_idx_check):
                        if neighbor_idx not in self.population:
                            ...
                            self._activate_point(neighbor_idx, warm_start_params=best_warm_start_params, serial=serial)
